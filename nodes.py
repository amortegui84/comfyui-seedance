...
RES_FAST = ["720p", "480p"]          # Fast: no 1080p
RATIO    = ["16:9", "9:16"]


# --------------------------------------------------------------------------- #
# API Key node — holds credentials, connects to all generation nodes
# --------------------------------------------------------------------------- #
...
class _I2VBase:
    ...
    def generate(self, api, first_frame, prompt, resolution, duration, seed,
        ...
        payload = {
            ...
            "content":      content,
            "resolution":   resolution,
            "ratio":        "adaptive",   # auto-detected from input image
            "duration":     duration,
            "watermark":    watermark,
            "camera_fixed": camera_fixed,
        }
...
class SeedanceI2V(_I2VBase):
    """Seedance 1.0 Pro — Image to Video, first frame + optional last frame (480 / 720 / 1080p)."""
    RESOLUTIONS = RES_PRO
    MODEL_ID    = "doubao-seedance-1-0-pro-250528"


class SeedanceI2VFast(_I2VBase):
